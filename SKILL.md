...
本机另有 Staryears 网关专用入口 `scripts/codex_staryears.py`，命令与状态语义和 `codex_deepseek.py` 一致：

- 只注册 `deepseek-v4.1-flash`（思考程度固定 `max`）；Provider id 仍为 `deepseek`，base_url 为 `https://api.staryears.net/v1`；
- 系统凭据 target 独立（`codex-staryears-deepseek-api-key`），Provider auth 只回调该入口自身；
- 模型目录不联网获取，使用随附模板 `resources/staryears-models.json`；
- 两个入口互斥（同一个 Provider 与角色文件）：切换前先对另一个入口执行 `uninstall`。

安装、验收命令、差异与限制见仓库根目录 `ADAPTATION.md`。
